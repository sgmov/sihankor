...
flowchart TB
    subgraph Core["司衡核心脉络"]
        ...
        Tao["道<br/><small>设计原则：被建构的</small>"]
...
4. 编号与分类：为每个子主张分配编号，标注其命题类型（工程必要性/规范性/描述性/定义性）
...
| 分类错误     | 将规范性主张当作工程必要性提取                                     | 先归类的检验延后到第二段 |
...
5. 层级归属确认：最终确认主张属于道（工程必要性）、法（方法论原则）还是术（具体实践）
...
一个主张声称自己属于道层（工程必要性），应通过以下清单：

- [ ] 命题类型明确：是工程必要性（what must be），不是规范性（what should be）
...
flowchart TD
    subgraph SelfExam["九段式自指检验"]
        S1["1. 主张提取<br/>'九段式是检验道层主张的有效方法'"] --> S1R["结果：规范性主张（应该用）<br/>非工程必要性<br/>不应进入道层"]
...
- **1. 主张提取**："反推九段式是检验道层主张的有效方法"
  -> 这是一个规范性主张（"应该用九段式检验"），不是工程必要性：所以它不应进入道层，而应定位为方法论规则
...
九段式的自我定位：九段式不是道，不是法：它是鉴，属于方法论层面。它不是"被建构的工程必要性"，而是"被发明并经过验证的检验工具"。它的有效性不依赖于哲学证明，而依赖于工程验证：五维天道 0/21 幸存证明了它具有区分力。
...
flowchart LR
    ...
    subgraph Results["验证结果"]
        ...
        R2["消除描述性/<br/>规范性混淆"]
        R3["从'必须理解'（规范性）<br/>校准为'代码自晦'<br/>（工程必要性）"]
...
背景：司衡早期体系将代码工程的道表述为五个独立的维度：运行之道、结构之道、演化之道、制约之道、映射之道。这五个维度被视为五条独立的工程必要性。
...
flowchart TD
    ...
    S5 --> S6["$6 逻辑一致性<br/>范畴归位：原主张是<br/>'应该理解'（规范性）<br/>非'理解不可跳过'（工程必要性）"]
    S6 --> S7["$7 可证伪条件<br/>不经理解即可维护的案例<br/>-> 已被 $4 满足"]
    S7 --> S8["$8 证伪判定<br/>正确示例： 原主张被推翻"]
    S8 --> S9["$9 校准建议<br/>-> '代码自晦，意图必复'<br/>（工程必要性，非规范性）"]
...
5. 第六段：原主张是"应该理解"（规范性），不是"理解不可跳过"（工程必要性）：范畴归位
6. 第七段：可证伪条件：若存在不经理解即可维护的案例，已被步骤 4 满足
7. 第八段：证伪：原主张被推翻
8. 第九段：校准："代码自晦，意图必复"（工程必要性陈述，非规范性要求）
