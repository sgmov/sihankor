...
- **触发判据**：空白发现——现有 L-01~L-13 仅覆盖文档生命周期，道三「代码自晦，意图必复」对代码的断言在法层无对应约束。这是法层对代码域的覆盖空白
- **发现人**：SiHankor 体系自检
- **发现时间**：260616
- **间隙描述**：代码术层约束（如禁止 unwrap、要求 pub API 文档化）的合法性仅基于工程惯例，无法追溯至道。法必须覆盖代码域，否则道的断言是不可执行的修辞
- **修正**：新增 L-14~L-19（代码术层约束的法层规则），§4.2 补充法层覆盖域声明。修正完成，stage 退回 3/3
- **修正依据**：[code-canon-derivation 鉴九段检验](../proposals/260616-2130-code-canon-derivation.sih.md)
...
文档归纳合成（术层实现）：从多份 source 文档归纳为一份 compendium 时，归纳本身是意图的再次编码（道二的递归适用）。每次归纳必须保持可追溯到原文的 resolve_ref 路径：归纳者的解读不得扭曲原始文档的含义。resolve_ref 是术层实现细节，服务于顺因之法，不是独立的法层概念。
...
| 编码 | 中文 | 英文      | 含义                                             | 下游引用            |
| ---- | ---- | --------- | ------------------------------------------------ | ------------------- |
| 1/3  | 草案 | Draft     | 意图明确但未经审查                               | 不可引用            |
| 2/3  | 审查 | Review    | 已过初步审查，核心论点稳定                       | 可引用但需注明阶段  |
| 3/3  | 定稿 | Ratify    | 收敛完成，当前认可版本                           | 可引为可靠依据      |
| 0/   | 替代 | Supersede | 已被新版本取代，后随 successor id（如 0/new-id） | 可引用，engine 标记 |
| X    | 废弃 | Deprecate | 概念或方向被放弃                                 | 禁止引用，无后继    |

`stage` 字段对所有 .sih.md 文档统一。编码形式相同，语义由文档所在目录（nature）决定：

| nature（目录） | stage 1/3                            | stage 2/3                        | stage 3/3    | stage 0                        | stage X |
| -------------- | ------------------------------------ | -------------------------------- | ------------ | ------------------------------ | ------- |
| spec           | 起草中，意图明确但未经审查，不可引用 | 审查中，已过初步审查，可谨慎引用 | 定稿，可依赖 | 已被替代，successor 指向新文档 | 废弃    |
| proposal       | 提案中                               | 决议中                           | 已决议       | 已被替代                       | 废弃    |
| decision       | 草拟                                 | 审查中                           | 定稿         | 已被替代                       | 废弃    |
| reference      | 起草中                               | 审查中                           | 定稿         | 已被替代                       | 废弃    |

spec/proposal/decision/reference 的 stage 回答"这份文档在多大程度上可以被下游信任"。note 的 stage 回答"这份洞察在多大程度上已被验证有效"。编码形式复用（1/3, 2/3, 3/3, 0, X），语义由 nature 提供解释上下文。

编码格式与 id 语法见[《司衡文档约定》](../engineering/SiHankor-Document-Conventions.sih.md)。

#### 目录即身份：：废除 type 字段

文档身份由其所在目录唯一确定。引擎从路径第一层推断 nature，不依赖 frontmatter 的 type 声明。

| 目录               | nature    | 中文     | 定义                                                           |
| ------------------ | --------- | -------- | -------------------------------------------------------------- |
| `specs/`           | spec      | 系统定义 | 回答"系统是什么"。意图草案（1/3）和正式规约（3/3）同在此处     |
| `proposals/`       | proposal  | 变更提议 | 回答"我们提议改变什么"。upstream 指向被变更的 spec 或 decision |
| `decisions/`       | decision  | 决策记录 | 回答"为什么这样选"。源于 proposal 的决议                       |
| `reference/`       | reference | 参照标准 | 回答"术语/概念精确指什么"。供查阅，不求证                      |
| `knowledge/notes/` | note      | 实践洞察 | 回答"我们学到了什么"。从工程实践中提炼的洞察                   |

设计约束：

- 目录即身份。不需要 type 字段。引擎从路径推断，人类从目录浏览。
- 同一目录下所有文档共享一个 nature。子目录用于领域细分，不改变 nature。
- `knowledge/drafts/` 中的文件无固定身份（非 .sih.md），不在治理范围内。
...
终止规则。spec/proposal/decision/reference 均可终止。终止是 stage 变更：需 ADR 记录终止原因。终止后文档迁移至对应 archived 目录（$6.2），stage 变为 X。note 同样适用终止规则（stage X），此外 note 的衰退由 verified 过期管理（$6.2）。
...
例外：引擎自动触发的变更不需 ADR：L-11 被动取消（后继死亡自动恢复）。这些不以人的判断为决策点。
...
| L-14 | 领域逻辑和公开 API 须可追溯至 ratify 规约；内部胶水代码豁免                                | 引用     | 道一（发散须收敛）                 | 顺因      |
| L-15 | pub 项须声明意图（doc comment）；trait impl 中 trait 已声明契约的豁免                      | 术层准入 | 道二（因果方向）+ 道三（意图必复） | 顺因      |
| L-16 | 间隙不得隐藏为裸 panic；逻辑上已证明安全的 expect 须加注 `// by construction`              | 术层准入 | 道四（间隙必管理）                 | 有度      |
| L-17 | 调试痕迹不得残留；二进制程序的操作性 I/O 豁免                                              | 术层准入 | 道四（临时桥梁非永久建筑）         | 知止      |
| L-18 | 不可达路径优先类型消除；exhaustive match 中的 unreachable 安全网允许；todo! 仅开发期可接受 | 术层准入 | 道三（代码自晦）+ 知止             | 知止      |
| L-19 | deny 级术层约束须经鉴检验（段一至段八，SURVIVES/CALIBRATE）方可生效                        | 术层准入 | 鉴（诗理区分）                     | 有度      |
...
- **文档域**（L-01~L-13）：治理文档生命周期——propose/resolve/ratify/reopen/supersede。法源从道二（因果方向：意图先于文档）、道三（意图必复：同一意图不应产生多份文档）、道四（间隙必管理：Reopen 机制）推导
- **代码域**（L-14~L-19）：治理代码术层约束的合法性——代码→规约追溯、意图声明、间隙暴露、deny 约束准入。法源从道一（收敛必为：代码须收敛于规约）、道二（因果方向：API 声明先行于实现）、道三（代码自晦，意图必复）、道四（间隙不得隐藏为 panic）推导

两域同属法层，同受鉴检验。代码域规则的详细推导与鉴九段检验见 [code-canon-derivation](../proposals/260616-2130-code-canon-derivation.sih.md)。
...
示例：知止之法的实践检验：限制 notes/ 不经过正式治理流程。检验：notes/ 中是否出现了本应在 specs/ 中的系统性定义？如果出现了，说明知止的边界需要重新校准（可能某些洞察应转化为 spec），而不是知止之法本身错误。
...
文档体系由以下目录构成。一级目录定义文档身份（nature），引擎从路径第一层推断。
...
| 不放什么 | 为什么选这个方案（-> decisions/）、这个方案还处于讨论中（-> proposals/）、构思碎片（-> knowledge/drafts/） |
| stage    | 1/3（起草，不可引用）-> 2/3（审查中，可谨慎引用）-> 3/3（定稿，可依赖）                                    |
| 格式     | 1/3 可自由描述，2/3 需结构化，3/3 需完整覆盖                                                               |
| 谁创建   | 工程师撰写；非技术用户口述需求，AI 代理生成                                                                |
| 子目录   | 按领域。如 `specs/payment/`、`specs/user/`                                                                 |

specs/ 下所有文档的 nature 为 spec，无论 stage 是 1/3 还是 3/3。同一份文档从起草到定稿，位置不变：：stage 在 frontmatter 中变化，不在文件系统中变化。

#### proposals/ ：： 变更提议

| 维度     | 内容                                                                                                        |
| -------- | ----------------------------------------------------------------------------------------------------------- |
| 放什么   | 方案对比、技术探索、RFC。回答"我们是否应该做 X"或"A 和 B 哪个更好"。不放方向路线图（-> specs/engineering/） |
| 不放什么 | 已决定的方案（-> decisions/）、已定稿的系统定义（-> specs/）、大方向路线图（-> specs/engineering/）         |
| stage    | 1/3（提案中）-> 2/3（决议中）-> 3/3（已决议）-> X（否决）                                                   |
| 格式     | 1/3 自由描述。2/3 需 ADR 三段式（背景/决策/后果）                                                           |
| 谁创建   | 任何角色。非技术用户：描述问题或想法，AI 起草提案                                                           |
| 子目录   | 按时间。如 `proposals/2026/`                                                                                |

proposal 的目标是改变 spec。当 proposal 被采纳（3/3），产出 decision（记录决策理由），spec 被修改。proposal 本身不变成 spec。
...
| 维度     | 内容                                                                       |
| -------- | -------------------------------------------------------------------------- |
| 放什么   | 项目术语表、概念纲要、命名约定、编码标准。回答"这个东西在我们项目里叫什么" |
| 不放什么 | 概念的哲学论证（-> specs/）、为什么选这个标准（-> decisions/）             |
| stage    | 通常 3/3（静态参照）。如需修订，走 Reopen                                  |
...
knowledge/ 下分两个子目录，承担不同的知识孵化职能：

##### knowledge/drafts/ ：： 构思碎片

| 维度     | 内容                                                  |
| -------- | ----------------------------------------------------- |
| 放什么   | 头脑风暴记录、方案草图、未成形的想法。非 .sih.md 格式 |
| 不放什么 | 已有固定身份的文档（-> 对应目录）                     |
| 格式     | 自由格式，无 frontmatter                              |

design：drafts 中的文件不需要 id、stage、upstream：：它们不是 .sih.md 文档。当构思足够清晰时，人将其转化为 .sih.md 并移入对应目录（specs/ 或 proposals/）。这部分在 engine 治理范围之外。

##### knowledge/notes/ ：： 实践洞察

| 维度     | 内容                                                                       |
| -------- | -------------------------------------------------------------------------- |
| 放什么   | 工作过程中产生的洞察：踩坑记录、可复用模式、被反复问到的问题、临时解决方案 |
| 不放什么 | 已晋升为正式规范的（-> specs/decisions/）、闲聊或一次性信息                |
| stage    | 1/3->2/3->3/3（note 的 stage 表达生命周期成熟度，编码复用）                |
| 格式     | 自由格式。AI 可生成草稿，人类通过 verified 字段确认洞察有效                |
| 谁创建   | AI 或人类。人类通过更新 verified 字段确认洞察的有效性                      |

note 有 stage。note 的核心字段为 id、stage 与 verified。stage 表达洞察的成熟度（1/3 初现->2/3 人类确认->3/3 已充分验证），verified 记录最近确认日期（格式 YYMMDD）。

note 的成熟转化：当 note 的洞察足够成熟时，人创建新的 spec/proposal/decision 文档（有 stage）来承载其核心内容。原 note 晋升至 stage 3/3 表示洞察已充分验证。身份变更（note->spec/decision）创建新文档，新文档在 deps 中引用原 note 作为前置知识。

衰减与衰退：note 的 verified 字段超过 `review_after_days`（在 config.yml 中定义）未更新时，engine 标记衰退警告。衰退的 note 在检索中被降权，但不迁移文件。人可通过重新审视并更新 verified 日期来解除衰退。衰退不等于废弃：旧洞察可能仍包含有价值的前置知识。

#### archive/ ：： 废弃文档

| 维度     | 内容                                                |
| -------- | --------------------------------------------------- |
| 放什么   | stage X（废弃）或 stage 0（被替代后无人引用）的文档 |
| 不放什么 | 活跃文档                                            |
| 子目录   | 按原目录。如 `archive/specs/`、`archive/proposals/` |

所有目录中 stage X 的文档，迁移至 `docs/archive/{原目录}/{name}.X.md`。归档时文件名追加 `.X` 后缀仅用于人类浏览识别废弃状态：文档 id 不变，engine 按 stage 字段判断，不依赖文件名。

#### 上游语义判据

文档归属的互斥性由"这份文档的 upstream 指向什么"决定：
...
| （无 upstream，通过引用链计算） | `knowledge/notes/`  | 它的上游是产生洞察的工作上下文 |
| 无                              | `knowledge/drafts/` | 构思碎片无固定身份             |

#### 子目录的通用规则

- 所有目录均允许子目录，最多三层。拆分阈值：单目录文件数 > 30。
- 推荐拆分维度见上表。knowledge/notes/ 不建议子目录：：通过引用链检索，不通过目录浏览。
- 拆分不改变文档的 stage、id，不影响引擎索引。

### 6.3 目录自定义

五类文档语义边界（追问、stage 范围、upstream 语义）是法层定义：：新增或删除一个类别意味着治理模型的变更，需走法层修正流程。目录名是术层约定：：在 `.sih/config.yml` 中声明映射即可。
...
### 6.4 glossary 与 semantic

#### glossary/ ：： 跨人类语言翻译

glossary/ 回答"同一个概念在不同人类语言中精确指什么"。中文（源语言）-> 英文（工程通用语）。

**创建条件**：项目需要多语言协作时才创建。单语言项目不创建此目录。
...
glossary/
  ...
  en.yml      # 目标语言映射（mapping + rejected + disambiguation + verified）
```

`zh.yml` 中每个条目声明 `derives-from`，指向 reference/ 中的文档 id。engine 通过 `zh.yml -> derives-from -> reference/` 做概念溯源，不需要独立的 `_concepts.yml` 概念注册表。
...
**因果方向**：`specs/` -> `reference/` -> `glossary/zh.yml` -> `glossary/en.yml`。reference 变更必须传播到 glossary，glossary 变更不反向影响 reference。跨语言方向上，zh.yml 的 definition 是语义权威源，en.yml 不重新定义概念：：en.yml 只提供翻译映射与歧义消解。

#### semantic.yml ：： 意图与代码的语义翻译

semantic 是引擎内建能力，排放于 `.sih/semantic.yml`。回答"这个业务意图在代码中对应哪些标识符？代码是否偏离了意图？"

**创建条件**：每个项目都需要。道四不可逃：：有规约就有间隙，有间隙就需要追踪。

```yaml
# .sih/semantic.yml
# 引擎维护。人类通过 engine MCP 工具查看和编辑。
# 每个条目的 derives-from 指向 specs/ 或 reference/ 中的一个文档 id。

退款超时:
  derives-from: 240602-1500-payment-spec#24h-refund
  code:
    - symbol: RefundTimeout
      location: internal/payment/config.go:42
      type: duration
      fidelity: direct
    - symbol: REFUND_DEADLINE_HOURS
      location: internal/payment/constants.go:15
      type: const
      fidelity: partial
      gap: "毫秒 vs 秒单位不一致，且未与 RefundTimeout 同步更新"
  verified: 2026-06-15
```

engine 从 semantic.yml 中执行：

- 正向查询：给定 spec 中的业务概念，找到代码中对应的所有标识符
- 反向查询：给定代码符号，找到它编码的业务意图
- 间隙检测：`fidelity: broken` 或 `partial` 的条目自动进入待治理清单
- 变更影响分析：spec 修改了某概念的定义 -> 所有关联 code 标记为 drifted

semantic.yml 的完整 schema 和 engine 索引实现在 engine 设计规范中定义。
