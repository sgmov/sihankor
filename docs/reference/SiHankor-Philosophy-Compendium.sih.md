...
| 常见误解                          | 正确理解                                                                                                                       |
|  |  |
| "间隙不可消除" = "间隙不可缩小" [X] | 好的规约可以减少间隙，好的实现可以逼近规约，但零间隙在逻辑上不可能                                                             |
| "承认不完备" = "可以不遵守" [X]     | 道四要求的是显式声明不完备和正规纠错路径，而非放任                                                                             |
| "自我指涉" = "自我否定" [X]         | 道四不否定治理的有效性：：它否定的是治理的绝对正确性。一个承认自己可能出错的治理引擎，比一个声称自己永远正确的治理引擎更值得信任 |
...
| 在司衡中的体现 | spec-coding、upstream 溯源、iCL 明->晰递进                 |
...
| 在司衡中的体现 | knowledge/drafts 构思不在治理范围、tags 不参与逻辑、三域边界、propose 可死亡        |
...
| 域  | 含义 | 司衡权限 |
| --- ||  |
| 治理域   | docs/ 内部        | 完全管理权（读/写/验证/驱动）  |
| 观察域   | config 声明的外部路径 | 只读访问（约取索引、验证合规） |
| 不可见域 | 未声明的路径          | 完全不可见                     |
...
**定义**：文档、代码、索引是治理过程的可观测产出。形迹层的核心属性是**可追溯性**：：任何形迹必须能通过 upstream 链追溯到其上游意图。
