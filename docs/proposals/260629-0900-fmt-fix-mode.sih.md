...
V-F-01 校验 `id` 字段格式：`YYMMDD-HHMM[-NNN]-语义短名`。当前 6 条 Fatal 规则中，V-F-01 是唯一一个**修复路径完全确定**的规则：当 `id` 缺少 HHMM 段时，从文件 mtime 取 HHMM 插入即可，不需要人工判断。
...
`--cascade` 默认 dry-run，必须显式 `--confirm` 才写盘。建议沿用 rebuild-stale-prune 提案规划的 dry-run / confirm 双阶段模式（该接口尚未落地，见 DEPS）。
...
`SystemTime` 是 UTC，id 中的 HHMM 约定为本地时间（agent 创建文档时所在时区）。`Cargo.toml` 已依赖 `chrono`（含 serde feature），Local 时区随默认 clock feature 提供，可直接用于时区转换，无需新增依赖。
