...
行迹原型 T1-T5 揭示了设计提案与实际使用之间的偏差。以下为修正后的结构：

每条行迹记录包含四类字段：

**锚定字段**（将行迹绑定到结构文档的特定位置）：

| 字段 | 内容 | 示例 |
| ---- | ---- | ---- |
| trace_id | 行迹唯一标识 | 260628-1900-trail-t1 |
| anchor_doc | 锚定的结构文档 ID | 260627-1100-jian-on-verification |
| summary | 一句话摘要 | 修复顺序从 DSR 优先改为工程集成优先 |

**转折字段**（记录转折本身）：

| 字段 | 内容 | 示例 |
| ---- | ---- | ---- |
| trace_type | 行迹类型 | direction_shift / divergence / decision_origin / discovery |
...
| 字段 | 内容 | 示例 |
| ---- | ---- | ---- |
| agents_involved | 参与 agent 的异构配置标识 | DS-v4-pro / GLM-5.1（可选） |
...
- divergence（分歧记录）：多 agent 交叉验证中的根本分歧及收敛过程
- decision_origin（决策来源）：某个结构文档中的结论是单一 agent 产出还是多 agent 共识
- discovery（间隙发现）：跨治理系统的格式间隙、工程约束揭示的设计缺口等非决策类发现。原型中的 T3（Stage 格式发现）和 T5（Archive Nature 缺口）属于此类
...
```rust
#[tool(description = "[SiHankor] Record a trail entry (intent turning point / divergence / decision origin / cross-system discovery)")]
...
参数映射到数据结构：anchor_doc（锚定文档 ID）、type（方向转折/分歧/决策来源/发现）、summary（一句话摘要）、turning_point（转折描述）、rationale（转折理由）、consequences（后果）、agents_involved（可选 agent 标识）。
...
iCL.analyze(doc) 产出（扩展后）:
  ...
  + trail_context: {
      ...
      trail_count: usize,
      has_open_trails: bool,  // 是否存在 stage < 3/3 的关联行迹
    }
```
...
行迹机制本身受知止约束。以下准入门槛决定是否推进到 L2：

- record_trail 在 5 次以上真实场景中被调用且产生有用行迹 -> 推进 MCP 工具测试覆盖
- iCL trail_context 在 3 次以上 analyze 调用中被使用且纳入 Cognition -> 推进 iCL 扩展的正式测试
- 两个门槛都未达到 -> 维持 L5（零实现）状态，工具接口设计作为候选存在
- 连续 3 个 DSR 周期后行迹数不足 10 条且无有效回溯 -> 降级行迹机制的优先级
