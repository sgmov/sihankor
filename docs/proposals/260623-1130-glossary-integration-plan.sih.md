...
Builder 用户在四个追问面前全部回答"我不知道"。追问引擎的问题中夹杂中英文术语：文档类型（nature）、成熟度（stage）、上游文档（upstream）、顺因（Shun-Yin：因果链可追溯）、有度（You-Du：力度匹配）、知止（Zhi-Zhi：知道不做什么），不熟悉司衡哲学的用户无法理解。
...
4. "中文（English）"格式的有效性假设未经 Builder 用户验证。如果 Builder 仍不理解"类型（nature）"，计划本身的效用等于零。需要在 P0 实现后进行二次人工测试。
