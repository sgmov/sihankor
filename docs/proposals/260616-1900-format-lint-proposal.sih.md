...
stage: 1/3
upstream: 260616-1800-plan-semantic-split-decision
---
...
format-lint 的法源是 Document-Conventions $八，而 Document-Conventions 的法源又是 Canon L-04（「文档格式纳入治理约束」）。定位如下：

```mermaid
flowchart LR
    Canon["Canon 法层：L-04 格式约束"] --> Conv["Document-Conventions 术层：$八 格式规则"]
...
| 规则 ID | 来源 | 级别 | 说明 |
|---------|------|------|------|
| C-01 | §8.5 | Error | 存在 emoji 或非 ASCII/CJK 字符 |
| C-02 | §8.5 | Error | em-dash `--` (U+2014) 出现在非代码块 |
| C-03 | §8.5 | Error | curly/smart quotes 出现 |
| C-04 | §8.5 | Warning | 箭头使用（提醒，有时正文中 `→` 可接受） |
| C-05 | §8.4 | Error | `---` 出现在正文（非 frontmatter 边界） |
| C-06 | §8.8 | Warning | 列表嵌套超过 2 层 |
| C-07 | §8.7 | Warning | 疑似 ASCII 图：≥3 连续行的行首为 ASCII 图形字符 |
| C-08 | §8.9 | Warning | 中英文在同一 token 内混合 |
| C-09 | §8.3 | Warning | 代码块无语言标签（复用 G-05 逻辑） |
| C-10 | §8.4 | Error | 表格列数 > 3（复用 G-04 逻辑） |
