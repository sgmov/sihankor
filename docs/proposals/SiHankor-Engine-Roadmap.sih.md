...
flowchart LR
    ...
    subgraph Reference["参照层"]
        R1["Onomastic-Philosophy<br/>3/3"]
...
- id/stage 必填字段验证
- upstream 对 spec/proposal/decision/reference 必填校验
...
1. **Compendium 1/3 → 2/3**：已完成。概念定义与五论对齐，交叉引用完整。

2. **Arguments 1/3 → 2/3**：已完成。论证案例与 Tao/Assay/Canon 章节对应完整。

3. **Onomastic-Philosophy 2/3 → 3/3**：已完成。命名体系无矛盾，代号全部 ratify。
