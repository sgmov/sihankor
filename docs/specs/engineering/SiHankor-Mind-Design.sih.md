...
> 注：本规范为工程设计文档（stage 1/3，type: mapping）。Mind 尚未实现，本文描述的是设计蓝图而非已实现的功能。实现时以 `src/main.rs` 中的实际代码为权威来源。本文的非治理性底层设计（如具体 Rust struct 字段）在实现过程中可能调整。待 Mind 实现完成后推进 stage 至 2/3。
