...
(4) 的输出可以触发新一轮 (1)：：例如发现某个文档的意图定位需要人类确认后，用确认结果重新分析。
...
> 注：本规范为工程设计文档（stage 3/3，在 specs/engineering/ 下，nature 为 spec）。Mind 已实现：iCL/iWW/iCT 三机 + 4 MCP 工具（analyze_document/propose_decision/verify_decision/full_analysis），53 tests pass，~850 LOC（mind 模块）。实现以 `src/main.rs` 中的实际代码为权威来源。
