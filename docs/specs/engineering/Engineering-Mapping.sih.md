...
> 本文档逐条追踪司衡哲学概念到工程实现的映射状态。基于 V3 R4 审计发现的 L3-L5 断裂，诚实记录每条映射链的当前状态、代码证据与偏差。所有 L 级别基于实际源码验证，与初步判断有差异处已标注说明。
...
道一的可证伪条件要求度量"产出方差"与"协调覆盖率"。代码中无任何方差度量管道。`ICL::diagnose_divergences`（icl.rs 第 236 行）使用启发式诊断：标题相似度判断重复、引用缺失判断断裂。这是发散的症状检测，不是道一定义的产出方差度量。道一作为五法共通之源，其验证方法在工程层完全缺失。
...
道四b要求纵向数据检验间隙增长，需在多个时间点测量治理与实践间隙。代码中无跨版本一致性检查、无规则数量时间序列追踪、无纵向审计机制。`V-G-08`（validator.rs 第 441 行）检查 stage X 文档不应被引用，这是废弃标记检查，非间隙增长度量。
...
iCT `check_shunshi`（ict.rs 第 300 行）实现时势维度：3/3 禁用暧昧措辞（第 306 行），1/3 禁用强制措辞（第 313 行），root 保护（第 322 行），stage X 保护（第 342 行）。时势维度有近似实现，故评为 L2。但 2/3 恒返回 Conditional（第 354 行）为硬编码，地势与人势维度完全缺失。
...
`Stage` 枚举（models.rs 第 7 行）定义 Propose/Resolve/Ratify/Deprecated/Superseded 五态。`is_referenceable`（第 48 行）返回 Resolve/Ratify，`is_terminal`（第 52 行）返回 Deprecated/Superseded。`from_str`（第 56 行）解析字符串到枚举。

偏差在于：`is_valid`（models.rs 第 46 行）恒返回 true，无转换条件校验。无 1/3 到 2/3 的递进约束，无 3/3 不可回退约束。`V-F-03`（validator.rs 第 244 行）调用 `doc.stage.is_valid()` 检查 stage 合法性，但因 `is_valid` 恒 true，此规则实际无校验效果。状态机存在但转换条件不完整。
...
最大断裂在道一的产出方差度量（链 1，L5）。道一是五法共通之源，其可证伪条件要求度量产出方差，但工程层零实现。这导致知止（G1）与有度（G3）的验证方法失去度量基础：G1 的"治理投入与方差成正比"无方差数据，G3 的"规则数与风险成正比"无风险度量。五法中两法的验证方法因道一度量缺失而无法执行。

次要断裂在 G1-G5 度量管道（链 6-10，均 L5）与鉴九段式（链 11，L5）。法论定义的五条工程验证方法全部缺失，iCT 的五法检验为降格实现（L2-L3），不构成法论定义的度量管道。鉴论自身声明零实现，九段式仅存于哲学文档。
...
stage 转换条件（链 13）。`is_valid` 恒返回 true，可通过增加转换矩阵（1/3->2/3->3/3 单向递进，3/3 不可回退）实现。当前缺失是实现遗漏。
...
- models.rs：Stage 枚举（第 7 行）、ViolationSeverity 枚举（第 182 行）、Document 结构（第 128 行）
- ict.rs：ICT 五法检验、build_dao_trace（第 386 行）
- iww.rs：IWW 决策建议生成
- icl.rs：ICL 认知分析（第 30 行）、上游链追溯（第 66 行）、发散诊断（第 236 行）
- grilling.rs：GrillingEngine 追问引擎、build_dao_trace（第 331 行）
- governance.rs：MCP 服务、full_analysis 三机流转
