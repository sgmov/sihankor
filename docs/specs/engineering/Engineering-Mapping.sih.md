...
- **已修复**：`project_status`（governance.rs 第 285 行起）在 2026-06-28 之前将 `fatal_violations_total` 硬编码为 0，导致 `compute_snapshot_diff` 的 `fatal_violations_delta` 永远为 0（两端都是 0，diff 永远是 0）。该 bug 已于 2026-06-28 修复（plan: plan_f990b08d）：现改为聚合最近 1000 条 `ValidationCompleted` 记录的 `fatal_count` 之和写入 ProjectSnapshot。
- 顺带修复：`project_status` 写入 metrics 表的 payload 由 `MetricEvent` enum 包装格式 `{"ProjectSnapshot":{...}}` 改为平铺 JSON `{"total_docs":...,...}`，与 `metrics.rs::ProjectSnapshotPayload` 反序列化结构对齐；`query_metrics` 排序增加 `id DESC` 次级键，确保 `created_at` 相同时 `records[0]` 是最新记录（修复前 created_at 相同时 SQLite 排序不稳定，导致 `compute_snapshot_diff` 偶发返回 `None` 或 delta 方向反转）。
- 偏差维度数：4（不含上述硬编码 bug 维度）。原 deliverable（2026-06-28 全量评估）计 5（含硬编码 bug），修复后降至 4。
- 测试覆盖：6 个集成测试（`tests/snapshot_diff_fatal_hardcode_test.rs`）覆盖单条/多条 fatal_count 累加、零 fatal、空 metrics、snapshot_diff delta 反映变化、增量方向性。原 6 个单测保持通过（3 metrics + 3 governance）。
...
偏差可量化为三项。`modifies_upstream` 使用字符串包含匹配判断是否修改上游（ict.rs 第 94 行），非语义级分析。`would_create_cycle`（ict.rs 第 523 行）已从桩函数提升为语法级字符串匹配：检查 action 描述是否引用 upstream_chain 中的文档 ID，引用则判为环路。此为近似实现（L2 而非 L1）——在 iCT 纯函数无 db 引用的约束下，字符串匹配是可行方案。无代码级追溯，仅文档到文档的引用链检查。
...
`V-F-03`（validator.rs 第 351 行）已从编码合法性检查（调用 `is_valid`，恒 true，无校验效果）升级为终态保护（调用 `is_terminal`，阻断对 Deprecated/Superseded 文档的修改）。`advance_stage`（project_state.rs 第 96 行）已集成 `can_transition_to` 校验，调用者需传入 `from` 和 `to` 两个 `Stage` 引用，非法转换在调用时即返回 Err。
...
偏差可量化为两项。`V-G-09`（缺失 decided-by，validator.rs 第 465 行）仅为 Guideline 级警告，非 Fatal 阻断，2/3 以上 decision 缺失 decided-by 不被强制拦截。`validate_governance` 依赖 `file_path` 推断 nature（第 466 行），当 file_path 为 None 时 nature 推断失败，decided-by 检查被跳过。governance.rs 第 171 行 `get_document` 已修复：从 `doc.nature` 构造合成路径 `docs/{nature}/doc.md` 传入 `validate_document`。此方案有一诚实性限制：`doc.md` 为虚构文件名，`infer_nature` 仅依赖目录结构不依赖文件名故能正常工作，但若未来代码依赖 file_path 做文件名级别判断，此虚构值会产生隐蔽 bug。稳健方案需在 Document 结构中存储实际 file_path（涉及 schema 变更），当前合成路径为合理 MVP。
...
当前零实现。行迹机制的写入由治理层 MCP 工具（`record_trail`）承担，读取由 iCL 在意图定位中纳入（`trail_context` 扩展至 Cognition，通过 `search_content` 按 `anchor_doc` 搜索关联行迹）。知止边界：只记录方向性转折，不记录探索过程细节。行迹准入不是机械判断，由 LLM 辅助层评估准入条件（方向性转折、可重复遭遇、非已被覆盖），确定性引擎机械裁决排除条件（ADR/应论已覆盖），人类治理者最终决定。行迹机制自身的知止验证：record_trail 经 5 次以上真实调用后推进测试覆盖，iCL trail_context 经 3 次以上 analyze 调用后推进正式测试，连续 3 个 DSR 周期累积不足 10 条行迹则降级优先级。数据成熟度为待验证。行迹以 .sih.md 文档形式存在于 knowledge/trails/ 目录，对开发者、外部审阅者、社区贡献者披露。
...
stage 转换条件（链 13，已修复）。`V-F-03` 已从编码合法性检查（调用 `is_valid`，恒 true）升级为终态保护（调用 `is_terminal`，阻断终态文档修改）。`advance_stage` 已集成 `can_transition_to` 校验。

decided-by 路径覆盖（链 14）。`get_document` 已修复：从 `doc.nature` 构造合成路径传入 `validate_document`。剩余限制：`doc.md` 为虚构文件名，若未来代码依赖 file_path 做文件名级别判断会产生隐蔽 bug。

顺因环检测（链 7）。`would_create_cycle` 已从桩函数提升为语法级字符串匹配（检查 action 描述是否引用 upstream 文档 ID），在 iCT 纯函数约束下为可行方案。L2 而非 L1。
