...
行迹机制是道三"代码自晦，意图必复"在机制层的直接展开。spec-coding 记录意图的产出形态（规范），行迹记录意图的形成过程（方向性转折及其理由）。行迹的道层授权来自道二（意图形成过程属于代码工程活动）和道三（行迹是意图恢复的原材料）。

当前零实现。行迹机制的写入由治理层（MCP 工具）承担，读取由 iCL 在意图定位中纳入。iCL 的扩展不违反其设计原则：仍然只读不写，信息来源从文档扩展到文档加行迹。知止边界：只记录方向性转折，不记录探索过程细节。数据成熟度为待验证，待跨 agent 实践积累后再决定完整工程化。
