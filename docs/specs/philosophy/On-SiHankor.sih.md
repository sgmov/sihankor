...
本文提供精简而精确的哲学表达。读完本文，可不参阅其他文档而大致知晓司衡的哲学全貌。各概念的详细论证、反推检验记录、工程实现细节，见 metadata 文件夹内其他文档。
...
```mermaid
flowchart TD
    D1["道一：发散自然，收敛必为<br/>多认知源意图发散"] --> D2["道二：意图先于代码<br/>意图有损编码为代码"]
...
- 术语表维护（glossary）：确保同一概念在所有文档和翻译中一致
- 翻译规则分离（i18n/translation.en.yml）：翻译规则与翻译内容分离
- .po 翻译流水线：标准 gettext 流程，支持 CI 校验
- LLM 自动化：通过提示词约束确保 LLM 翻译合道

### 7.2 三域边界与三阶生命周期

三域边界模型是知止之法的工程实现：

- **治理域**：sih-docs/ 内部：完全管理权
- **观察域**：scope.yaml 声明的路径：只读访问
- **不可见域**：未声明的路径：完全不可见

三阶生命周期是顺势之法的工程实现：`propose（议）-> resolve（决）-> ratify（定）`，单向不可逆，修正通过叠加而非覆盖。propose 可以死亡：不是所有提议都值得推进。
...
- **《司衡论》（本文）**：总纲：破题、立义、贯脉络。首次接触，建立全貌
- **《司衡鉴论》**：检验方法论：反推九段式、诗与理区分、可证伪条件。需要理解“司衡凭什么可信”时
- **《司衡哲学纲要》**：全部核心概念的权威定义。需要精确理解某个概念时
- **《司衡哲学论证集》**：每个概念的反推检验记录。需要了解“为什么司衡信这些”时
- **《司衡工程映射》**：哲学到工程的完整映射。需要理解哲学如何落地时
- **《道之上的元》**：元层次的追问式阐述。需要深入理解元时

### 9.2 阅读路径

- **首次接触**：通读本文 -> 《哲学纲要》$一~$四（道与法的核心定义）
- **查找定义**：《哲学纲要》文末“术语速查表”
- **深入理解**：本文 -> 《哲学纲要》-> 《论证集》对应章节
- **工程实现**：本文 $七 -> 《工程映射》 -> 具体 spec 文档
