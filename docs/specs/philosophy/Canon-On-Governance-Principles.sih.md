...
> **校准 2026-06-28**：J（矩）原定义为"精确判定，pass/fail"。工程实践发现此定义混淆了两个独立维度——判定方式与阻断行为。可机械判定的检查中包含风格性规则（如列表嵌套层数），强行 pass/fail 产生噪声阻断，违反有度（低风险高力度）。经校准，J 的"精确判定"特性保留——J 级规则的判定方式仍然是精确机械判定——但阻断行为从"pass/fail 阻断"调整为"静默记录，仅计数不阻断"。校准标准来自 G3（有度：力度与风险匹配——J 级属低风险风格性检查，应对应低力度静默记录）；校准程序来自 G2（顺因：哲学层显式承认并重新定义，代码不单方面修改规约）。
