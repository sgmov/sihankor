---
id: 2406101030-on-sihankor-canon
stage: 3/3
...
spec/proposal/decision/reference 的 stage 回答"这份文档在多大程度上可以被下游信任"。编码形式复用，语义由 nature 提供解释上下文。note 无 stage：note 是实践洞察的沉淀，有 id 和 verified 字段，不受生命周期治理。
...
终止规则。spec/proposal/decision/reference 均可终止。终止是 stage 变更：需 ADR 记录终止原因。终止后文档迁移至对应 archived 目录（$6.2），stage 变为 X。note 无 stage，不适用终止规则：note 的衰退由 verified 过期管理（$6.2）。
...
0/ 文档的 stage 值直接编码了后继文档 ID（如 0/2406101030-new-doc），无需单独的 successor 字段。这一要求来自顺因之法：读者看到 stage 即知"意图去了哪里"。
...
| 维度     | 内容 |
| -------- | ---- |
| 放什么   | 方案对比、技术探索、RFC。回答"我们是否应该做 X"或"A 和 B 哪个更好"。也存放 plan 型文档（路线图、任务拆解） |
...
| stage    | -（无 stage。note 是实践洞察，不受生命周期治理） |
| 格式     | 自由格式。AI 可生成草稿，人类通过 verified 字段确认洞察有效 |
| 谁创建   | AI 或人类。人类通过更新 verified 字段确认洞察的有效性 |

note 无 stage。note 的核心字段为 id 与 verified。verified 记录洞察被确认的日期（格式 YYMMDD），是 note 可信度的唯一时间锚点。

note 的成熟转化：当 note 的洞察足够成熟时，人创建新的 spec/proposal/decision 文档（有 stage）来承载其核心内容。原 note 本身不变：不是 stage 推进，而是身份变更（note→spec/decision）。新文档在 deps 中引用原 note 作为前置知识。
...
退款超时:
  derives-from: 2406021500-payment-spec#24h-refund
  code:
...
- 2606131728-sihankor-onomastic-philosophy
  - 命名中英代号对照
