...
| note | 草稿（draft） | 活跃（active） | 已晋升（promoted） | 已衰减（decayed） | 废弃 |

spec/proposal/decision/reference 的 stage 回答"这份文档在多大程度上可以被下游信任"。note 的 stage 回答"这份洞察处于什么生命周期阶段"。编码形式复用，语义由 nature 提供解释上下文。
...
| stage    | 1/3（草稿）→ 2/3（活跃，经人类确认）→ 3/3（晋升，内容迁移至 specs/ 或 decisions/，原 note 清退）→ 0/decayed（衰减）→ X（终止） |
| 格式     | 自由格式。AI 可生成 1/3 草稿，但必须人类确认（变更为 2/3）才能进入活跃状态 |
| 谁创建   | AI 草稿 + 人类确认 |

note 的 stage 是生命周期，不是可信度。1/3 = 草稿，2/3 = 活跃（人类已确认），3/3 = 已晋升（内容迁移，原 note 清退），0 = 衰减（过 `review_after_days` 未晋升），X = 终止。

note 的晋升：engine 检测到引用计数 >=3 且跨 >=2 目录时，**建议晋升**，不自动推进 stage。晋升是人的决策——人决定 note 的核心内容以何种形式迁移到 specs/ 或 decisions/，并附 ADR 记录晋升理由。

衰减：engine 标记 stage 为 `0/decayed`，移入 `knowledge/notes/` 中排除常规检索（或标记 `decayed: true`）。
...
所有目录中 stage X 的文档，迁移至 `docs/archive/{原目录}/{name}.X.md`。
...
**因果方向**：`specs/` → `reference/` → `glossary/zh.yml` → `glossary/en.yml`。reference 变更必须传播到 glossary，glossary 变更不反向影响 reference。
